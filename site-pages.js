(function () {
    ...
    /// Le jour de la dernière révision des textes légaux.
    ///
    /// **Une seule source.** La date était recopiée dans les onze traductions de `ui`,
    /// et n'avait donc jamais été remise à jour depuis le 6 mai : onze endroits à
    /// changer, c'est zéro endroit changé. Elle se formate ici selon la langue lue.
    var LEGAL_UPDATED = '2026-08-31';

    function updatedLabel(lang) {
        var jour = new Date(LEGAL_UPDATED + 'T00:00:00Z');
        var etiquettes = {
            fr: 'Mise à jour : ', en: 'Updated: ', es: 'Actualizado: ', it: 'Aggiornata: ',
            de: 'Aktualisiert: ', ja: '更新日：', ko: '업데이트: ', 'zh-Hans': '更新：',
            uk: 'Оновлено: ', ru: 'Обновлено: ', ar: 'آخر تحديث: '
        };
        var locales = { fr: 'fr-FR', en: 'en-GB', es: 'es-ES', it: 'it-IT', de: 'de-DE',
            ja: 'ja-JP', ko: 'ko-KR', 'zh-Hans': 'zh-Hans', uk: 'uk-UA', ru: 'ru-RU', ar: 'ar' };
        var texte;
        try {
            texte = jour.toLocaleDateString(locales[lang] || 'en-GB',
                { year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' });
        } catch (e) { texte = LEGAL_UPDATED; }
        return (etiquettes[lang] || etiquettes.en) + texte;
    }
    ...
    var appFeatures = {
        ...
        dvffrance: {
            fr: ["Carte des prix : une pastille par commune, puis chaque vente à son adresse et le plan cadastral.", "Prix médians par type de bien et par millésime, avec quartiles, nombre de ventes et surface médiane.", "Fiche de quartier : écoles et résultats aux examens, commerces nommés, transports, risques et qualité de l’air.", "Application gratuite financée par la publicité, sans compte ni achat intégré."],
            en: ["Price map: one pill per town, then every recorded sale at its address and the cadastral plan.", "Median prices by property type and year, with quartiles, sale counts and median floor area.", "Neighbourhood profile: schools and exam results, named shops, transport, hazards and air quality.", "Free app funded by ads, with no account and no in-app purchase."],
            es: ["Mapa de precios: una etiqueta por municipio, luego cada venta en su dirección y el plano catastral.", "Precios medianos por tipo de bien y año, con cuartiles, número de ventas y superficie mediana.", "Ficha de barrio: colegios y resultados, comercios con nombre, transporte, riesgos y calidad del aire.", "App gratuita financiada con publicidad, sin cuenta ni compras integradas."],
            it: ["Mappa dei prezzi: una pastiglia per comune, poi ogni vendita al suo indirizzo e il piano catastale.", "Prezzi mediani per tipo di immobile e annata, con quartili, numero di vendite e superficie mediana.", "Scheda di quartiere: scuole e risultati, negozi con nome, trasporti, rischi e qualità dell’aria.", "App gratuita finanziata dalla pubblicità, senza account né acquisti in-app."],
            de: ["Preiskarte: ein Punkt je Gemeinde, dann jeder erfasste Verkauf an seiner Adresse und der Katasterplan.", "Medianpreise nach Objektart und Jahrgang, mit Quartilen, Verkaufszahl und Medianfläche.", "Viertelprofil: Schulen und Prüfungsergebnisse, benannte Geschäfte, Verkehr, Risiken und Luftqualität.", "Kostenlose App, durch Werbung finanziert, ohne Konto und ohne In-App-Kauf."],
            ja: ["価格マップ：市町村ごとのピル表示から、登録された各取引の所在地、そして地籍図まで。", "物件種別と年次ごとの中央価格。四分位、取引件数、中央面積を併記。", "周辺情報：学校と試験結果、店名入りの商店、交通、災害リスク、大気質。", "広告で運営される無料アプリ。アカウント登録もアプリ内課金もありません。"],
            ko: ["가격 지도: 지자체별 알약 표시에서 등록된 각 거래의 주소, 그리고 지적도까지.", "부동산 유형과 연도별 중앙 가격. 사분위수, 거래 건수, 중앙 면적 포함.", "동네 정보: 학교와 시험 결과, 이름이 있는 상점, 교통, 위험, 대기질.", "광고로 운영되는 무료 앱. 계정도 앱 내 구매도 없습니다."],
            'zh-Hans': ["价格地图：先按市镇显示价签，再到每笔已登记交易的具体地址和地籍图。", "按房产类型和年份的中位价格，附四分位数、成交数量和中位面积。", "街区档案：学校与考试成绩、有名称的商铺、交通、风险和空气质量。", "由广告支持的免费应用，无需账户，无应用内购买。"],
            uk: ["Мапа цін: спершу позначка на кожну комуну, далі кожен зареєстрований продаж за адресою та кадастровий план.", "Медіанні ціни за типом житла та роком, із квартилями, кількістю продажів і медіанною площею.", "Профіль кварталу: школи та результати іспитів, магазини з назвами, транспорт, ризики та якість повітря.", "Безкоштовний застосунок на рекламі, без облікового запису та без покупок усередині."],
            ru: ["Карта цен: сначала метка на каждую коммуну, затем каждая зарегистрированная сделка по адресу и кадастровый план.", "Медианные цены по типу жилья и году, с квартилями, числом сделок и медианной площадью.", "Профиль квартала: школы и результаты экзаменов, магазины с названиями, транспорт, риски и качество воздуха.", "Бесплатное приложение на рекламе, без учётной записи и без покупок внутри."],
            ar: ["خريطة الأسعار: علامة لكل بلدية، ثم كل عملية بيع مسجلة في عنوانها والمخطط العقاري.", "الأسعار الوسيطة حسب نوع العقار والسنة، مع الأرباع وعدد المبيعات والمساحة الوسيطة.", "بطاقة الحي: المدارس ونتائج الامتحانات، المتاجر بأسمائها، النقل، المخاطر وجودة الهواء.", "تطبيق مجاني يموّل بالإعلانات، دون حساب ودون شراء داخل التطبيق."]
        },
    };

    var appOrder = ['carbufrance', 'irvefrance', 'toilettefrance', 'defibfrance', 'dvffrance', 'mactuner'];
    ...
    var typeText = {
        ...
        realestate: {
            fr: { data: "La position sert à ouvrir la carte là où vous êtes, situer le quartier décrit et calculer les distances aux écoles et aux commerces. Les ventes mises de côté ou écartées, les communes récemment consultées, la langue, le thème et les relevés téléchargés restent sur l’appareil.", source: "Les prix viennent des demandes de valeurs foncières publiées par la Direction générale des finances publiques et géolocalisées par Etalab. S’y ajoutent la carte des loyers du ministère du Logement, les diagnostics de performance énergétique de l’ADEME, les risques et arrêtés de catastrophe naturelle de Géorisques, le plan cadastral de l’IGN, l’annuaire de l’éducation et les indicateurs de résultats du ministère de l’Éducation nationale, les faits enregistrés publiés par le service statistique du ministère de l’Intérieur, le découpage IRIS de l’INSEE, le raccordement à la fibre et les antennes mobiles publiés par l’Arcep, et les commerces et transports d’OpenStreetMap.", support: [["Pourquoi ma commune n’a-t-elle aucun prix ?", "Sous cinq ventes exploitables, aucune médiane n’est affichée : ce serait une anecdote et non un indicateur. L’Alsace, la Moselle et Mayotte ne figurent pas du tout dans ces données, elles relèvent d’un autre régime de publicité foncière."], ["Le millésime en cours affiche peu de ventes.", "L’administration publie deux fois par an : l’année en cours est toujours partielle et ses derniers mois manquent. Ce n’est pas un effondrement du marché."], ["Est-ce une estimation de mon bien ?", "Non. L’application applique à la surface indiquée les prix réellement enregistrés autour du point choisi. Elle ne visite pas le bien et ignore son état, son étage et sa vue, qui font une grande part du prix."]] },
            en: { data: "Location is used to open the map where you are, place the neighbourhood being described and measure distances to schools and shops. Sales you set aside or dismiss, recently viewed towns, language, theme and downloaded survey data all stay on the device.", source: "Prices come from the property transfer records published by the French public finances directorate and geocoded by Etalab. Added to these are the rent map of the Ministry of Housing, energy performance certificates from ADEME, hazards and natural-disaster orders from Géorisques, the cadastral plan from IGN, the schools directory and results indicators from the Ministry of Education, recorded offences published by the Interior Ministry statistical service, the INSEE IRIS boundaries, fibre broadband coverage and mobile masts published by Arcep, and shops and transport from OpenStreetMap.", support: [["Why does my town show no price?", "Below five usable sales no median is shown: it would be an anecdote, not an indicator. Alsace, Moselle and Mayotte are absent from this data altogether, as they fall under a different land registration regime."], ["The current year shows very few sales.", "The administration publishes twice a year: the current year is always partial and its most recent months are missing. It is not a market collapse."], ["Is this a valuation of my property?", "No. The app applies prices actually recorded around the chosen point to the floor area you enter. It does not visit the property and knows nothing of its condition, floor or view, which account for much of the price."]] },
            es: { data: "La ubicación sirve para abrir el mapa donde estás, situar el barrio descrito y calcular distancias a colegios y comercios. Las ventas guardadas o descartadas, los municipios consultados, el idioma, el tema y los datos descargados permanecen en el dispositivo.", source: "Los precios proceden de las declaraciones de valores inmobiliarios publicadas por la dirección general de finanzas públicas francesa y geolocalizadas por Etalab. Se añaden el mapa de alquileres del ministerio de Vivienda, los certificados energéticos de ADEME, los riesgos y decretos de catástrofe natural de Géorisques, el plano catastral del IGN, el directorio de centros educativos y los indicadores de resultados del ministerio de Educación, los hechos registrados publicados por el servicio estadístico del ministerio del Interior, la división IRIS del INSEE, la cobertura de fibra y las antenas móviles publicadas por Arcep, y los comercios y transportes de OpenStreetMap.", support: [["¿Por qué mi municipio no muestra precio?", "Con menos de cinco ventas utilizables no se muestra ninguna mediana: sería una anécdota, no un indicador. Alsacia, Mosela y Mayotte no figuran en estos datos, se rigen por otro régimen registral."], ["El año en curso muestra pocas ventas.", "La administración publica dos veces al año: el año en curso siempre es parcial y faltan sus últimos meses. No es un desplome del mercado."], ["¿Es una tasación de mi vivienda?", "No. La app aplica a la superficie indicada los precios realmente registrados alrededor del punto elegido. No visita el inmueble e ignora su estado, planta y vistas, que pesan mucho en el precio."]] },
            it: { data: "La posizione serve ad aprire la mappa dove ti trovi, situare il quartiere descritto e calcolare le distanze da scuole e negozi. Le vendite messe da parte o scartate, i comuni consultati di recente, la lingua, il tema e i rilievi scaricati restano sul dispositivo.", source: "I prezzi provengono dalle dichiarazioni di valore immobiliare pubblicate dalla direzione generale delle finanze pubbliche francese e geolocalizzate da Etalab. Si aggiungono la mappa degli affitti del ministero dell’Abitazione, gli attestati di prestazione energetica dell’ADEME, i rischi e i decreti di catastrofe naturale di Géorisques, il piano catastale dell’IGN, l’annuario scolastico e gli indicatori di risultato del ministero dell’Istruzione, i fatti registrati pubblicati dal servizio statistico del ministero dell’Interno, la ripartizione IRIS dell’INSEE, la copertura in fibra e le antenne mobili pubblicate da Arcep, e i negozi e trasporti di OpenStreetMap.", support: [["Perché il mio comune non mostra alcun prezzo?", "Sotto le cinque vendite utilizzabili non viene mostrata alcuna mediana: sarebbe un aneddoto, non un indicatore. Alsazia, Mosella e Mayotte non figurano affatto in questi dati, seguono un altro regime di pubblicità immobiliare."], ["L’annata in corso mostra poche vendite.", "L’amministrazione pubblica due volte l’anno: l’anno in corso è sempre parziale e mancano i suoi ultimi mesi. Non è un crollo del mercato."], ["È una stima del mio immobile?", "No. L’app applica alla superficie indicata i prezzi realmente registrati attorno al punto scelto. Non visita l’immobile e ignora stato, piano e vista, che pesano molto sul prezzo."]] },
            de: { data: "Der Standort dient dazu, die Karte dort zu öffnen, wo du bist, das beschriebene Viertel zu verorten und Entfernungen zu Schulen und Geschäften zu berechnen. Zurückgelegte oder ausgeblendete Verkäufe, zuletzt aufgerufene Gemeinden, Sprache, Design und heruntergeladene Erhebungen bleiben auf dem Gerät.", source: "Die Preise stammen aus den von der französischen Finanzverwaltung veröffentlichten Grundstücksverkaufsmeldungen, georeferenziert von Etalab. Hinzu kommen die Mietkarte des Wohnungsministeriums, Energieausweise der ADEME, Risiken und Katastrophenerlasse von Géorisques, der Katasterplan des IGN, das Schulverzeichnis und die Ergebnisindikatoren des Bildungsministeriums, die vom Statistikdienst des Innenministeriums veröffentlichten erfassten Straftaten, die IRIS-Gliederung des INSEE, die von der Arcep veröffentlichte Glasfaser- und Mobilfunkabdeckung sowie Geschäfte und Verkehr aus OpenStreetMap.", support: [["Warum zeigt meine Gemeinde keinen Preis?", "Unter fünf verwertbaren Verkäufen wird kein Median angezeigt: das wäre eine Anekdote, kein Indikator. Elsass, Mosel und Mayotte fehlen in diesen Daten vollständig, sie unterliegen einem anderen Grundbuchrecht."], ["Der laufende Jahrgang zeigt sehr wenige Verkäufe.", "Die Verwaltung veröffentlicht zweimal jährlich: das laufende Jahr ist immer unvollständig, die letzten Monate fehlen. Das ist kein Markteinbruch."], ["Ist das eine Bewertung meiner Immobilie?", "Nein. Die App wendet auf die angegebene Fläche die tatsächlich erfassten Preise rund um den gewählten Punkt an. Sie besichtigt nichts und kennt weder Zustand noch Etage oder Aussicht, die einen großen Teil des Preises ausmachen."]] },
            ja: { data: "位置情報は、現在地に地図を開き、説明している地区を特定し、学校や店舗までの距離を計算するために使われます。取っておいた売買、除外した売買、最近見た市町村、言語、テーマ、ダウンロードした調査データは端末に残ります。", source: "価格はフランス公共財政総局が公開する不動産取引記録に由来し、Etalabが位置情報を付与しています。これに住宅省の家賃地図、ADEMEのエネルギー性能診断、Géorisquesの災害リスクと自然災害令、IGNの地籍図、国民教育省の学校名簿と成果指標、内務省統計局が公開する認知件数、INSEEのIRIS区分、Arcepが公開する光ファイバー敷設状況と携帯基地局、OpenStreetMapの商店と交通情報が加わります。", support: [["市町村に価格が表示されないのはなぜですか。", "利用可能な取引が5件未満の場合、中央値は表示しません。それは指標ではなく逸話だからです。アルザス、モゼル、マヨットはこのデータに一切含まれません。別の不動産登記制度に属するためです。"], ["現在の年次の取引件数が少ないのですが。", "行政は年2回公開します。現在の年次は常に部分的で、直近の数か月が欠けています。市場の崩壊ではありません。"], ["これは私の物件の査定ですか。", "いいえ。アプリは入力された面積に、選んだ地点の周辺で実際に登録された価格を当てはめるだけです。物件を訪問せず、価格の大部分を左右する状態、階、眺望を知りません。"]] },
            ko: { data: "위치는 현재 있는 곳에 지도를 열고, 설명 중인 동네를 지정하며, 학교와 상점까지의 거리를 계산하는 데 사용됩니다. 따로 보관하거나 제외한 거래, 최근 조회한 지자체, 언어, 테마, 내려받은 조사 자료는 기기에 남습니다.", source: "가격은 프랑스 공공재정총국이 공개하는 부동산 거래 기록에서 오며 Etalab이 위치를 부여합니다. 여기에 주택부의 임대료 지도, ADEME의 에너지 성능 진단, Géorisques의 위험 및 자연재해 명령, IGN의 지적도, 교육부의 학교 명부와 성과 지표, 내무부 통계청이 공개하는 인지 건수, INSEE의 IRIS 구획, Arcep이 공개하는 광케이블 연결 현황과 이동통신 기지국, OpenStreetMap의 상점과 교통 정보가 더해집니다.", support: [["왜 우리 지자체에는 가격이 없나요?", "사용 가능한 거래가 다섯 건 미만이면 중앙값을 표시하지 않습니다. 지표가 아니라 일화가 되기 때문입니다. 알자스, 모젤, 마요트는 이 데이터에 전혀 포함되지 않으며 다른 부동산 등기 제도를 따릅니다."], ["현재 연도의 거래가 매우 적습니다.", "행정기관은 연 2회 공개합니다. 현재 연도는 언제나 부분적이며 최근 몇 달이 빠져 있습니다. 시장 붕괴가 아닙니다."], ["이것은 제 부동산 감정인가요?", "아닙니다. 앱은 입력한 면적에 선택한 지점 주변에서 실제로 등록된 가격을 적용할 뿐입니다. 부동산을 방문하지 않으며 가격의 상당 부분을 좌우하는 상태, 층, 조망을 알지 못합니다."]] },
            'zh-Hans': { data: "位置用于在你所在的地方打开地图、定位所描述的街区，并计算到学校和商铺的距离。你保留或排除的成交、最近查看的市镇、语言、主题以及已下载的调查数据都留在设备上。", source: "价格来自法国公共财政总局公布的不动产交易记录，由 Etalab 完成地理定位。此外还包括住房部的租金地图、ADEME 的能效诊断、Géorisques 的风险与自然灾害法令、IGN 的地籍图、国民教育部的学校名录与成绩指标、内政部统计部门公布的立案数据、INSEE 的 IRIS 分区、Arcep 公布的光纤覆盖情况与移动基站，以及 OpenStreetMap 的商铺与交通信息。", support: [["为什么我的市镇没有价格？", "可用成交少于五笔时不显示中位数：那只是个别案例，而非指标。阿尔萨斯、摩泽尔和马约特完全不在此数据中，它们适用另一套不动产登记制度。"], ["当前年份的成交量很少。", "行政部门每年公布两次：当前年份始终是不完整的，最近几个月尚未纳入。这并非市场崩塌。"], ["这是对我房产的估价吗？", "不是。应用只是把所选地点周边实际登记的价格套用到你输入的面积上。它没有实地查看，也不知道房屋状况、楼层和视野，而这些占了价格的很大一部分。"]] },
            uk: { data: "Позиція використовується, щоб відкрити мапу там, де ви є, визначити описуваний квартал і обчислити відстані до шкіл та магазинів. Відкладені чи вилучені продажі, нещодавно переглянуті комуни, мова, тема й завантажені зведення залишаються на пристрої.", source: "Ціни походять із декларацій вартості нерухомості, які публікує головне управління державних фінансів Франції та геолокує Etalab. До них додаються мапа орендних ставок міністерства житла, енергетичні сертифікати ADEME, ризики та накази про стихійні лиха від Géorisques, кадастровий план IGN, довідник закладів освіти та показники результатів міністерства освіти, зареєстровані правопорушення від статистичної служби міністерства внутрішніх справ, поділ IRIS від INSEE, покриття оптичним зв’язком і вежі мобільного зв’язку за даними Arcep, а також магазини й транспорт з OpenStreetMap.", support: [["Чому в моїй комуні немає ціни?", "Якщо придатних продажів менше п’яти, медіана не показується: це був би поодинокий випадок, а не показник. Ельзас, Мозель і Майотта взагалі відсутні в цих даних, бо підпадають під інший режим реєстрації нерухомості."], ["Поточний рік показує дуже мало продажів.", "Адміністрація публікує двічі на рік: поточний рік завжди неповний, останніх місяців бракує. Це не обвал ринку."], ["Це оцінка мого житла?", "Ні. Застосунок лише застосовує до вказаної площі ціни, реально зареєстровані навколо обраної точки. Він не оглядає житло й не знає його стану, поверху та виду, які значною мірою визначають ціну."]] },
            ru: { data: "Местоположение используется, чтобы открыть карту там, где вы находитесь, определить описываемый квартал и вычислить расстояния до школ и магазинов. Отложенные или исключённые сделки, недавно просмотренные коммуны, язык, тема и загруженные сводки остаются на устройстве.", source: "Цены берутся из деклараций стоимости недвижимости, публикуемых главным управлением государственных финансов Франции и геолоцируемых Etalab. К ним добавляются карта арендных ставок министерства жилья, энергетические сертификаты ADEME, риски и постановления о стихийных бедствиях от Géorisques, кадастровый план IGN, справочник учебных заведений и показатели результатов министерства образования, зарегистрированные правонарушения от статистической службы министерства внутренних дел, деление IRIS от INSEE, покрытие оптоволокном и вышки мобильной связи по данным Arcep, а также магазины и транспорт из OpenStreetMap.", support: [["Почему у моей коммуны нет цены?", "Если пригодных сделок меньше пяти, медиана не показывается: это был бы единичный случай, а не показатель. Эльзас, Мозель и Майотта вовсе отсутствуют в этих данных, поскольку подпадают под иной режим регистрации недвижимости."], ["Текущий год показывает очень мало сделок.", "Администрация публикует дважды в год: текущий год всегда неполный, последних месяцев не хватает. Это не обвал рынка."], ["Это оценка моего жилья?", "Нет. Приложение лишь применяет к указанной площади цены, реально зарегистрированные вокруг выбранной точки. Оно не осматривает жильё и не знает его состояния, этажа и вида, которые во многом определяют цену."]] },
            ar: { data: "يُستخدم الموقع لفتح الخريطة حيث أنت، وتحديد الحي الموصوف، وحساب المسافات إلى المدارس والمتاجر. تبقى المبيعات المحفوظة أو المستبعدة، والبلديات التي اطّلعت عليها مؤخرًا، واللغة، والمظهر، والبيانات المنزّلة على الجهاز.", source: "تأتي الأسعار من تصريحات القيم العقارية التي تنشرها الإدارة العامة للمالية العمومية الفرنسية ويحدّد Etalab مواقعها. تضاف إليها خريطة الإيجارات لوزارة الإسكان، وشهادات الأداء الطاقي من ADEME، والمخاطر ومراسيم الكوارث الطبيعية من Géorisques، والمخطط العقاري من IGN، ودليل المدارس ومؤشرات النتائج من وزارة التربية، والوقائع المسجلة التي تنشرها المصلحة الإحصائية لوزارة الداخلية، وتقسيم IRIS من INSEE، وتغطية الألياف البصرية ومحطات الهاتف المحمول التي تنشرها Arcep، والمتاجر والنقل من OpenStreetMap.", support: [["لماذا لا تعرض بلديتي أي سعر؟", "دون خمس مبيعات قابلة للاستخدام لا يُعرض أي وسيط: سيكون ذلك حكاية لا مؤشرًا. الألزاس والموزيل ومايوت غير موجودة في هذه البيانات إطلاقًا، فهي تخضع لنظام تسجيل عقاري آخر."], ["السنة الجارية تعرض عددًا قليلًا من المبيعات.", "تنشر الإدارة مرتين في السنة: السنة الجارية جزئية دائمًا وتنقصها أشهرها الأخيرة. ليس هذا انهيارًا للسوق."], ["هل هذا تقييم لعقاري؟", "لا. يطبّق التطبيق على المساحة التي تحددها الأسعار المسجلة فعليًا حول النقطة المختارة. إنه لا يعاين العقار ولا يعرف حالته ولا طابقه ولا إطلالته، وهي عوامل تشكّل جزءًا كبيرًا من السعر."]] }
        }
    };
    ...
    /* AVIF, WebP, puis l'original. Le PNG reste : c'est lui que servent les
       balises og:image de ces pages, et les aperçus des réseaux sociaux ne
       négocient pas de format. Le CSS pose picture { display: contents }. */
    function iconPicture(app, attrs) {
        var src = icon(app), base = src.replace(/\.png$/i, '');
        return '<picture>' +
    ...
    function header(lang, app, title, subtitle) {
        return '<header class="page-header site-page-header"><div class="page-header-inner"><a href="' + withLang('./index.html', lang) + '" class="page-icon-link" aria-label="' + ui[lang].home + '">' + iconPicture(app, ' alt="' + app.name + '" class="page-icon"') + '</a><div><p class="premium-eyebrow">' + escapeHTML(app.name) + '</p><h1 class="page-title">' + escapeHTML(title) + '</h1></div></div><p class="page-subtitle">' + escapeHTML(subtitle) + '</p></header>';
    }
    ...
    function adText(lang, app) {
        ...
        // Aucune des cinq apps iOS ne propose d'achat ni de don intégré : la
        // publicité est la seule ressource, et une vidéo récompensée offre six heures
        // sans publicité. C'est `app.premium` qui en décide, plus haut.
        var donations = {
        ...
        // **Le texte suit `premium`, pas le type d'app.** Le branchement portait sur le
        // domaine — toilettes, défibrillateurs, le reste — et rangeait donc CarbuFrance,
        // IRVEFrance et DVFFrance dans « le reste », à qui il faisait déclarer des achats
        // intégrés gérés par StoreKit. Aucune des cinq n'en propose : `premium: false`
        // partout, et le commentaire sous `donations` le disait déjà. Une politique de
        // confidentialité qui décrit un traitement inexistant est fausse, et c'est le
        // document que l'App Store publie.
        if (!app.premium) return ads[lang] + ' ' + donations[lang];
    ...
    function renderPrivacy(lang, app) {
        ...
        return header(lang, app, labels.privacy, app.name + ' — ' + updatedLabel(lang)) + '<main class="premium-page"><section class="page-card premium-hero-clean"><p class="premium-eyebrow">' + labels.privacy + '</p><h2 class="premium-heading">' + escapeHTML(app.name) + '</h2><p class="premium-lead">' + escapeHTML(commonCopy[lang].privacyLead) + '</p></section><section class="page-card">' + renderSections(sections) + '</section></main>' + footer(lang, app.name);
    }
